...
from src.utils.metrics import get_metrics_summary, get_prometheus_metrics, get_filtered_metrics_summary
...
async def get_metrics(
    endpoint_filter: Optional[str] = Query(None, description="Filter metrics by endpoint path"),
    time_range_minutes: Optional[int] = Query(None, description="Only include metrics from last N minutes"),
    include_system: bool = Query(True, description="Include system metrics (CPU, memory, disk)")
) -> Dict[str, Any]:
    """
...
async def get_health_metrics() -> Dict[str, Any]:
    ...
    return {
        ...
        "avg_response_time_ms": summary.get("performance", {}).get("avg_duration_ms", 0),
        "p95_response_time_ms": summary.get("performance", {}).get("p95_duration_ms", 0),
        ...
        ) if summary.get("system", {}).get("max_buffer_size", 0) > 0 else 0
    }