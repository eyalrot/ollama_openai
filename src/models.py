...
class OllamaOptions(BaseModel):
    ...
    typical_p: Optional[float] = Field(None, ge=0.0, le=1.0, description="Typical sampling parameter")
    repeat_last_n: Optional[int] = Field(None, description="Last n tokens to consider for repeat penalty")
    temperature: Optional[float] = Field(None, ge=0.0, description="Sampling temperature")
    repeat_penalty: Optional[float] = Field(None, description="Repeat penalty parameter")
    ...
    num_batch: Optional[int] = Field(None, description="Batch size for prompt processing")
    num_gqa: Optional[int] = Field(None, description="Number of GQA groups")
    num_gpu: Optional[int] = Field(None, description="Number of layers to offload to GPU")
...
class OllamaGenerateRequest(BaseModel):
    ...
    keep_alive: Optional[Union[str, int]] = Field("5m", description="Model keep-alive duration")
...
class OllamaChatRequest(BaseModel):
    ...
    messages: List[OllamaChatMessage] = Field(..., min_length=1, description="Chat messages")
    format: Optional[Literal["json"]] = Field(None, description="Response format")
    options: Optional[OllamaOptions] = Field(None, description="Model options")
    template: Optional[str] = Field(None, description="Prompt template")
    stream: bool = Field(True, description="Stream response")
...
class OpenAIMessage(BaseModel):
    ...
    role: Literal["system", "user", "assistant", "function", "tool"] = Field(..., description="Message role")
    ...
    def validate_content_or_function(self):
        ...
        if self.content is None and self.function_call is None and self.tool_calls is None:
            raise ValueError("Message must have either content, function_call, or tool_calls")
        return self
...
class OpenAIFunction(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(..., description="Function parameters as JSON Schema")
...
class OpenAIChatRequest(BaseModel):
    ...
    messages: List[OpenAIMessage] = Field(..., min_length=1, description="Input messages")
    ...
    # Function calling
    functions: Optional[List[OpenAIFunction]] = Field(None, description="Available functions (deprecated)")
    function_call: Optional[Union[str, Dict[str, str]]] = Field(None, description="Function call mode (deprecated)")
...
class OpenAIChatResponse(BaseModel):
    ...
    id: str = Field(default_factory=lambda: f"chatcmpl-{uuid.uuid4().hex[:8]}", description="Unique ID")
    object: Literal["chat.completion"] = Field("chat.completion", description="Object type")
    created: int = Field(default_factory=lambda: int(datetime.now().timestamp()), description="Creation timestamp")
    model: str = Field(..., description="Model used")
...
class OpenAIDelta(BaseModel):
    ...
    function_call: Optional[Dict[str, Any]] = Field(None, description="Function call delta")
...
class OpenAIStreamChoice(BaseModel):
    ...
    finish_reason: Optional[str] = Field(None, description="Finish reason when complete")
    logprobs: Optional[Dict[str, Any]] = Field(None, description="Log probabilities")
