...
class LoadTestRunner:
    ...
    async def run_load_test(self, config: LoadTestConfig) -> LoadTestResult:
        ...
        test_result = LoadTestResult(
            ...
            error_rate=result['errors'] / result['actual_requests'] if result['actual_requests'] > 0 else 0,
    ...
    async def _simulate_request(self, config: LoadTestConfig):
        ...
        async with track_request(config.endpoint, config.method, config.model) as metric:
    ...
    async def _simulate_streaming_request(self, config: LoadTestConfig):
        ...
        async with track_streaming_request(config.endpoint, config.method, config.model) as (metric, wrapper_factory):
            # Simulate processing time
            await asyncio.sleep(config.simulate_work_ms / 1000)
    ...
    async def _verify_metrics_accuracy(
        ...
        if config.use_streaming:
            streaming_requests = summary.get('streaming', {}).get('total_streaming_requests', 0)
            if expected_requests > 0:
                accuracy['streaming_count'] = min(streaming_requests / expected_requests, 1.0)
    ...
    def print_results(self, results: Dict[str, LoadTestResult]):
        ...
        # Summary table
        print(f"{'Test Name':<20} {'Requests':<10} {'RPS':<10} {'Avg RT(ms)':<12} {'P95 RT(ms)':<12} {'Errors':<8} {'Memory(MB)':<12}")
        print("-" * 100)
        
        for name, result in results.items():
            print(f"{name:<20} {result.actual_requests:<10} {result.requests_per_second:<10.1f} "
        ...
        for name, result in results.items():
            print(f"\n{name.upper()}:")
            print(f"  Requests: {result.actual_requests} (target: {result.config.total_requests})")
    ...
    def get_load_test_summary(self) -> Dict[str, Any]:
        ...
        avg_performance_degradation = statistics.mean(
            r.performance_degradation for r in self.results if r.performance_degradation > 0
        )
        ...
        return {
            ...
            "metrics_reliable": total_metrics_lost < total_requests * 0.01,  # Less than 1% loss
