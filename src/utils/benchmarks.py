...
class MetricsBenchmark:
    ...
    async def run_all_benchmarks(self, iterations: int = 1000) -> Dict[str, BenchmarkResult]:
        """
        ...
        results["streaming_tracking"] = await self._benchmark_streaming_tracking(iterations)
        
        # 4. Concurrent tracking
        results["concurrent_tracking"] = await self._benchmark_concurrent_tracking(iterations)
        ...
        for name, result in results.items():
            if name != "baseline" and self.baseline_result:
                result.overhead_percent = (
                    ...
                    self.baseline_result.avg_time_ms * 100
                )
    ...
    async def _benchmark_streaming_tracking(self, iterations: int) -> BenchmarkResult:
        ...
        async def streaming_operation():
            ...
            async with track_streaming_request("/test/stream", "POST", "stream-model") as (metric, wrapper_factory):
                metric.status_code = 200
    ...
    async def _benchmark_concurrent_tracking(self, iterations: int) -> BenchmarkResult:
        ...
        async def concurrent_operation():
            ...
            async def single_request(i):
                async with track_request(f"/test/concurrent_{i}", "POST", "concurrent-model") as metric:
    ...
    def print_results(self, results: Dict[str, BenchmarkResult]) -> None:
        ...
        # Print summary table
        print(f"{'Test Name':<20} {'Avg Time (ms)':<15} {'Overhead %':<12} {'Memory (MB)':<12}")
        print("-" * 80)
        
        for name, result in results.items():
            overhead_str = f"{result.overhead_percent:.2f}%" if result.overhead_percent > 0 else "baseline"
            print(f"{name:<20} {result.avg_time_ms:<15.3f} {overhead_str:<12} {result.memory_usage_mb:<12.2f}")
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        # Find max overhead
        max_overhead = max(r.overhead_percent for r in self.results if r.overhead_percent > 0)
        ...
        return {
            ...
            "baseline_avg_time_ms": self.baseline_result.avg_time_ms if self.baseline_result else 0,
