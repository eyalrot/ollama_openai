...
async def get_body_bytes(request: Request) -> bytes:
    ...
    if hasattr(request, '_body'):
        logger.debug(
            ...
            extra={"extra_data": {"request_id": request_id, "body_length": len(request._body)}}
        )
    ...
    if hasattr(request.state, 'body'):
        ...
        if isinstance(body, bytes):
            logger.debug(
                ...
                extra={"extra_data": {"request_id": request_id, "body_length": len(body)}}
            )
            return body
...
async def get_body_json(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            ...
            extra={"extra_data": {"request_id": getattr(request.state, "request_id", "unknown")}}
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
