...
class MetricsCollector:
    ...
    async def get_summary(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            if streaming_metrics:
                first_byte_times = [s.first_byte_time_ms for s in streaming_metrics if s.first_byte_time_ms > 0]
                throughputs = [s.throughput_bytes_per_second for s in streaming_metrics if s.throughput_bytes_per_second > 0]
                chunk_sizes = [s.avg_chunk_size for s in streaming_metrics if s.avg_chunk_size > 0]
                
                if first_byte_times:
                    streaming_stats["avg_first_byte_time_ms"] = sum(first_byte_times) / len(first_byte_times)
                if throughputs:
                    streaming_stats["avg_throughput_bytes_per_second"] = sum(throughputs) / len(throughputs)
                if chunk_sizes:
                    streaming_stats["avg_chunk_size"] = sum(chunk_sizes) / len(chunk_sizes)
    ...
    async def get_prometheus_metrics(self) -> str:
        ...
        for endpoint, stats in summary['endpoints'].items():
            ...
            lines.extend([
                ...
                f"# TYPE http_requests_per_endpoint_total counter",
                ...
                f"# TYPE http_request_duration_per_endpoint_seconds gauge",
                ...
                f"# TYPE http_request_errors_per_endpoint_total counter",
            ...
            if stats.get('streaming_requests', 0) > 0:
                lines.extend([
                    ...
                    f"# TYPE http_streaming_requests_per_endpoint_total counter",
    ...
    async def get_filtered_summary(
        ...
        async with self._lock:
            ...
            if endpoint_filter:
                metrics = [m for m in metrics if endpoint_filter.lower() in m.endpoint.lower()]
...
class StreamingResponseWrapper:
    ...
    def __init__(self, response_stream, metric: RequestMetrics, collector: MetricsCollector):
    ...
    async def _finalize_metrics(self):
        ...
        if self.chunk_sizes:
            self.metric.streaming.avg_chunk_size = sum(self.chunk_sizes) / len(self.chunk_sizes)
