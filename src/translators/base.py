...
class BaseTranslator(ABC, Generic[OllamaRequestType, OpenAIRequestType, OpenAIResponseType, OllamaResponseType]):
    """
    ...
    def __init__(self, model_mappings: Optional[Dict[str, str]] = None):
        ...
        self.logger.debug(f"Initialized {self.__class__.__name__} with {len(self.model_mappings)} model mappings")
    ...
    def translate_response(
        self, 
        openai_response: OpenAIResponseType, 
    ...
    def extract_options(self, ollama_options: Optional[OllamaOptions]) -> Dict[str, Any]:
        """
        ...
        for ollama_key, openai_key in mapping.items():
            if ollama_key in options_dict:
                ...
                if ollama_key == 'top_k' and openai_key == 'top_k':
                    ...
                    self.logger.debug(f"Skipping unsupported parameter 'top_k': {value}")
                    continue
