...
class TestDockerSecurity:
    ...
    def test_non_root_user(self):
        ...
        try:
            ...
            assert result.stdout.strip() == "proxyuser", "Container not running as proxyuser"
    ...
    def test_read_only_filesystem(self):
        ...
        result = subprocess.run(
            [
                ...
                "--read-only",
                "--tmpfs", "/tmp",
                "-v", f"{config_dir.absolute()}:/app/config:ro",
                "-e", "OPENAI_API_BASE_URL=http://test",
                "-e", "OPENAI_API_KEY=test",
                "ollama-proxy-test:prod"
            ],
...
class TestDockerCompose:
    ...
    @pytest.mark.skipif(
        not os.path.exists("/.dockerenv") and subprocess.run(["docker", "compose", "version"], capture_output=True).returncode != 0,
    ...
    def test_volume_permissions(self):
        ...
        result = subprocess.run(
            [
                ...
                "--name", container_name,
                "-v", f"{config_dir.absolute()}:/app/config:ro",
                "-v", f"{logs_dir.absolute()}:/app/logs",
                "-e", "OPENAI_API_BASE_URL=http://test",
                "-e", "OPENAI_API_KEY=test",
                "ollama-proxy-test:prod",
                "sleep", "30"
