...
from src.models import OllamaEmbeddingRequest, OpenAIEmbeddingResponse, OpenAIEmbeddingData, OpenAIUsage
from src.routers.embeddings import create_embeddings, router
from src.utils.exceptions import UpstreamError, ValidationError
...
class TestEmbeddingsEndpoint:
    ...
    async def test_create_embeddings_success(self, mock_translator, mock_client_context, mock_request, sample_ollama_request, sample_openai_response):
        ...
        mock_translator.validate_model_name.assert_called_once_with("text-embedding-ada-002")
    ...
    @pytest.mark.asyncio
    @patch('src.routers.embeddings.translator')
    async def test_create_embeddings_validation_error(self, mock_translator, mock_request, sample_ollama_request):
        """Test embeddings endpoint with validation error."""
        # Setup mock to raise ValidationError
        mock_translator.validate_model_name.side_effect = ValidationError("Invalid model")
    ...
    @patch('src.routers.embeddings.retry_client_context')
    @patch('src.routers.embeddings.translator')
    async def test_create_embeddings_upstream_error(self, mock_translator, mock_client_context, mock_request, sample_ollama_request):
    ...
    @patch('src.routers.embeddings.retry_client_context')
    @patch('src.routers.embeddings.translator')
    async def test_create_embeddings_timeout_error(self, mock_translator, mock_client_context, mock_request, sample_ollama_request):
    ...
    @patch('src.routers.embeddings.retry_client_context')
    @patch('src.routers.embeddings.translator')
    async def test_create_embeddings_connection_error(self, mock_translator, mock_client_context, mock_request, sample_ollama_request):
        ...
        mock_client.request_with_retry.side_effect = httpx.ConnectError("Connection failed")
    ...
    async def test_create_embeddings_batch_input(self, mock_request):
        ...
        with patch('src.routers.embeddings.retry_client_context') as mock_client_context, \
