...
def test_ssl_production_setup():
    ...
    try:
        result = subprocess.run(
            ["docker", "exec", "ollama-proxy-ssl", "ls", "-la", "/etc/ssl/certs/"],
            capture_output=True,
            text=True,
            timeout=10,
        )
        ...
        if result.returncode == 0 and "pem" in result.stdout.lower():
            ...
            print(
                f"   ✅ Read-only access: {':ro' in 'yes' if 'r-' in result.stdout[:20] else 'not verified'}"
            )
        else:
    ...
    working_models = ["google/gemma-2-9b-it:free", "mistralai/mistral-7b-instruct:free"]
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "ollama-proxy-ssl",
                "--format",
                "{{.HostConfig.ReadonlyRootfs}},{{.HostConfig.SecurityOpt}}",
            ],
            capture_output=True,
            text=True,
        )
        ...
        if "true" in result.stdout and "no-new-privileges" in result.stdout:
            print("   ✅ Container security: Read-only filesystem, no-new-privileges")
        else:
    ...
    try:
        ...
        print("   ✅ Resource limits: Memory: 1GB, CPU: 2 cores")
        print("   ✅ SSL certificates: Mounted read-only from host")
        print("   ✅ Secure communication: Ready for private SSL endpoints")
    except Exception as e:
